...
mess.groupData = mess.isGroup ? client.chats.get(mess.from) : undefined
//function
...
let clientGroupData = (groupMetadata.participants.find(res => res.jid == client.user.jid))
let isClientAdmin = clientGroupData.isAdmin||clientGroupData.isSuperAdmin
let senderGroupData = (groupMetadata.participants.find(res => res.jid == chatData.sender.jid))
let isSenderAdmin = senderGroupData.isAdmin || senderGroupData.isSuperAdmin
return {...groupMetadata,groupAdmins,isClientAdmin,clientGroupData,senderGroupData,isSenderAdmin}
}
let quotedMsgData = async() => {
if (!chatData.quotedMsg) return

...
let result_event = {eventName,_command: regCommand,command: command.map(tr => tr.toString()),callback,usedPrefix:true,enable:true,tags,info:'Tidak Ada Info',...opt}
...
findPrefix = findPrefix.sort((a,b) => b.toString().length - a.toString().length)
findPrefix = findPrefix[0]
var prefix = !event.usedPrefix ? event._command.find(tr => tr.test(msg.string)) && event._command.find(tr => tr.test(msg.string)).exec(msg.string) : findPrefix && findPrefix.exec(msg.string) || this.default_prefix.exec(msg.string)
...
cmds = cmds.concat((typeof msg.body == 'object' && event.stc && event.stc == msg.body.fileSha256) ? [event._command] : findCmd);
events.push(event)
}
...
event.callback(msg,{text,group,isUrl,isAdmin,query,command,commandNpref,prefix,usedPref,args,cmd,number,...msg,client:this.client,quotedMsg})
} catch (e){
if (!util.format(e).includes('this.isZero')){
let data = {response:'bad',status:null,event:event?.eventName,error: this.functions.util.format(e)};
console.log(data);

...
const fs = require('fs')
const ffmpeg = require('fluent-fd')
...
module.exports = class WAConnection {
  ...
  async reply(mess, message, opt) {
    ...
    return await this.sendMessage(typeof mess == 'object'?mess.key.remoteJid: mess, {
      ...prepared,
      quoted: typeof mess == 'object'?mess: null, ...opt
    })
  }
  ...
  async sendMessageFromContent(jid, obj, opt = {}) {
    let prepare = await this.generateWAMessageFromContent(jid, obj, opt)
    await this.relayMessage(prepare)
    return prepare
  }
  async fakeReply(jid, message, opt = {}, fakeJid, participant, fakeMessage) {
